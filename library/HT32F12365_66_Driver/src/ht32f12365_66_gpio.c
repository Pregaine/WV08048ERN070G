 ...
 *        @arg GPIO_DV_12MA : Selects output driving current as 12 mA
 *        @arg GPIO_DV_16MA : Selects output driving current as 16 mA
 * @retval None
 ************************************************************************************************************/
void GPIO_DriveConfig(HT_GPIO_TypeDef* GPIOx, u16 GPIO_PIN, GPIO_DV_Enum Drive)
{
  ...
  {
    if((GPIO_PIN & 0x0001) == 1)
    {
      temp = index << 1;
      CurrentMode |= ((u32) Drive << temp);
      PinPosition |= ((u32) 0x03  << temp);
    }
    GPIO_PIN >>= 1;
  }
 
  GPIOx->DRVR &= ~PinPosition;
  GPIOx->DRVR |= CurrentMode;
  GPIO_CK_OFF();
}
 ...
 * @param AFIO_PORT: GPIO_PA ~ GPIO_PE.
...
u32 _GPIO_ClockControl(HT_GPIO_TypeDef* GPIOx, ControlStatus Cmd)
{
  ...
  /*--------------------------------------------------------------------------------------------------------*/
  u32 offset = ((((u32)GPIOx) & 0x0000F000) >> 12) / 2 + 16;
