 ...
 *        @arg FLASH_WAITSTATE_3: three wait state.
 * @retval None
 ************************************************************************************************************/
...
FLASH_State FLASH_ErasePage(u32 PageAddress)
{
  ...
  do
  {
    if (uAddress - PageAddress == FLASH_PAGE_SIZE)
    {
  ...
  do {
  #endif
  status = FLASH_WaitForOperationEnd();
  #if (AUTO_CMDVERIFY == 1)
  if (status != FLASH_COMPLETE)
    break;
  } while (rw(uAddress - 4) != 0xFFFFFFFF);
  #endif
